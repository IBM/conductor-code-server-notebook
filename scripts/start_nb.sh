...
if [ -n "$CONDUCTOR_REST_URL" ]; then
	...
	# TODO: Need to handle SSL file creation potentially
else
...
host=`hostname -f`
export HOME=${DATADIR}
${CODESERVER_DEPLOY_DIR}/bin/code-server --install-extension ms-python.python --user-data-dir ${DATADIR}/data --extensions-dir ${DATADIR}/extensions
${CODESERVER_DEPLOY_DIR}/bin/code-server --auth none --disable-telemetry --bind-addr ${host}:8080 --user-data-dir ${DATADIR}/data --extensions-dir ${DATADIR}/extensions
