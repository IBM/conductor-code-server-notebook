...
declare -r CODESERVER_DATA_DIR="${NOTEBOOK_DATA_BASE_DIR}/${APP_NAME}/${APP_UUID}"
...
create_notebook_base_dir() {
    ...
    if [ ! -d "$CODESERVER_DATA_DIR" ]; then
        make_directory_with_multilevel_permission $CODESERVER_DATA_DIR 777 775
        chmod +t $CODESERVER_DATA_DIR
    else
        # Modify permissions.
        chmod u+rwx,g+rwxs,o+rwxt $CODESERVER_DATA_DIR
        # check write permission
...
copy_codeserver_package () {
    copy_files ./package/${CODESERVER_PACKAGE_NAME} $NOTEBOOK_DEPLOY_DIR/
    cd $NOTEBOOK_DEPLOY_DIR
    tar xvf ${CODESERVER_PACKAGE_NAME}
    rm -rf ${CODESERVER_PACKAGE_NAME}
}
...
deploy_notebook() {
    echo "Deploy code-server notebook to $NOTEBOOK_DEPLOY_DIR"
    create_notebook_deploy_dir
    create_notebook_base_dir
    create_service_logs_dir
    copy_scripts_dir
...
deploy_updated_notebook() {
    echo "NOTEBOOK_UPDATE_PACKAGE=true. Updating code-server notebook."   
    create_notebook_deploy_dir
    create_notebook_base_dir
    create_service_logs_dir
    copy_scripts_dir
